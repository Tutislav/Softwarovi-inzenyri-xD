					...
					if ($result->num_rows > 0) {				
						...
						while($row = $result->fetch_assoc()) {
							...
							echo "<tr id='detail_" .  $row["id_recenze"] . "_manage' style='display: none;'><td colspan='100%'><div>";
                   					echo "<p id='reviewer'><i class='fa fa-user'></i>Recenzent: </p>";
               						echo "<p id='article_title'><i class='fa fa-newspaper-o'></i>Článek: </p>";
							echo "<p id='author'><i class='fa fa-user'></i>Autor: </p>";
                   					echo "</div></td></tr>";
						}
					}
					echo "</table></div>";
